...
DECOMPOSE_SYSTEM_PROMPT = """You are a legal analyst specializing in Power of Attorney (POA) law in Qatar.
...
DECOMPOSE_PROMPT_TEMPLATE = """Analyze this Legal Brief and decompose it into legal sub-issues that need research.
...
{{
    ...
    "primary_question": "The main legal question to answer (in English)",
    "sub_questions": ["Supporting questions that help answer the main question (in English)"],
...
[
    {{
        ...
        "sub_questions": [
            "What is the scope of the grantor's authority per the Commercial Registry?",
            "Can a manager with limited authority delegate powers beyond their scope?"
        ],
        "relevant_facts": ["Grantor is Manager (Passports only)", "POA grants full management powers"],
...
class Decomposer:
    ...
    async def decompose(self, legal_brief: dict) -> list[dict]:
        """
        ...
        logger.info("Calling LLM to decompose legal brief...")

        response = await self.llm.chat(
            user_message=prompt,
            system_message=DECOMPOSE_SYSTEM_PROMPT,
            temperature=0.2
        )
